    ...
    def forward(self, src,
        ...
        for layer in self.layers:
            if self.return_atten_map:
                ...
                output_maps_list.append(output)
            else:
        ...
        if self.return_atten_map:
            return output, torch.stack(atten_maps_list), torch.stack(output_maps_list)
        else:
            return output
...
class Block(nn.Module):
    ...
    def forward(self, x):
        h = 32
        w = 24
        x = rearrange(x, '(h w) b c -> b c h w', h = h, w = w)  #768*2*256
...
class DepthWiseConv2d(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size, padding, stride = 1, bias = True):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(dim_in, dim_in, 7, stride=1, padding=9, groups=dim_in, dilation=3),
...
class FeedForwardwithdiaCNN(nn.Module):
    def __init__(self, dim, hidden_dim, dropout = 0.):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(dim, hidden_dim, 1),
...
class FeedForwardWithLargeK(nn.Module):
    def __init__(self, dim, hidden_dim, dropout = 0.):
        super().__init__()
        self.net = nn.Sequential(
...
class FeedForwardWithConvNeXt(nn.Module):
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=1024, dropout=0.1,
        ...
        self.ffn = FeedForwardWithLargeK(d_model, d_model*4, dropout=dropout)
        # self.ffn = FeedForwardWithConvNeXt(d_model, d_model*4, dropout=dropout)
        # self.ffn = Block(dim=d_model, drop_path=0.1)
        # self.ffn1 = Block(dim=d_model, drop_path=0.1)
        # self.ffn = FeedForwardwithdiaCNN(d_model, d_model*4, dropout=dropout)
    ...
    def forward_post(self,
        ...
        # src = src + self.dropout1(src2) #768*2*256
        # src = self.norm1(src)
...
class LocalPoseR(nn.Module):
    ...
    def __init__(self, block, layers, cfg, **kwargs):
        ...
        super(LocalPoseR, self).__init__()
        ...
        d_model = cfg.MODEL.DIM_MODEL   #256
        dim_feedforward = cfg.MODEL.DIM_FEEDFORWARD  #1024
        encoder_layers_num = cfg.MODEL.ENCODER_LAYERS #3
        n_head = cfg.MODEL.N_HEAD  #8
        pos_embedding_type = cfg.MODEL.POS_EMBEDDING #sin
        w, h = cfg.MODEL.IMAGE_SIZE #256 192
        ...
        self.bn2 =  nn.BatchNorm2d(num_features=768, eps=0, affine=False, track_running_stats=False)
        # self.bn2 =  nn.BatchNorm2d(256, affine=False)#, track_running_stats=False)
    ...
    def _make_position_embedding(self, w, h, d_model, pe_type='sine'):
        ...
        if pe_type == 'none':
            ...
            with torch.no_grad():
                self.pe_h = h // 8  #32
                self.pe_w = w // 8  #24
                length = self.pe_h * self.pe_w  #768
            if pe_type == 'learnable':  #this type is similar to ViT
    ...
    def _make_sine_position_embedding(self, d_model, temperature=10000,
        ...
        pos = pos.flatten(2).permute(2, 0, 1) #([1, 256, 32, 24])->([1, 256, 768])->768 1 256
    ...
    def forward(self, x):
        x = self.conv1(x)  #torch.Size([2, 3, 256, 192])
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        bs, c, h, w = x.shape #2 512 16 12
        x = x.flatten(2).permute(2, 0, 1)  #1*256*728---》192×1×512
        x = self.global_encoder(x, pos=self.pos_embedding)
        x = x.permute(1, 2, 0).contiguous().view(bs, c, h, w)
        x = self.deconv_layers(x) #2 512 16 12
...
def get_pose_net(cfg, is_train, **kwargs):
    ...
    model = LocalPoseR(block_class, layers, cfg, **kwargs)
